...
void UNO_AI_which_color_i_wanna_play(UNO_room* this_room,UNO_player* p1)//分析自己手牌，确定自己想出什么颜色牌，如果自己遇到黑牌选择情况，选择那个颜色
{
	...
	cout<<"AI思考要什么颜色\n";
	if(p1->card_color[0]==UNO_none)//如果自己最后一张是黑牌，打完后没牌了，则随便选个颜色
	{
	...
	int color_box[4];//4种颜色，存储颜色卡数量，因为颜色序号为1，2，3，4，所以存到0，1，2，3
		...
		p1->i_want_play_this_color=UNO_none;//清空原先想要的颜色
		for(i=0;i<4;i++)
			if(color_box[i]>color_number)//如果当前颜色数量大于最大数量，
			{
				...
				p1->i_want_play_this_color=i+1;//将当前想出的颜色修正为当前颜色
			}

		if(p1->i_want_play_this_color==UNO_none)//如果没获取到想要的颜色，说明可能自己有的颜色会导致别人赢，而自己没的颜色别人也可能没，所以选一个大家都没的颜色
		{
			for(int k=0;k<4;k++)
			{
				color_box[k]=0;
			...
			for(int i=0;i<UNO_member_limit;i++)
				...
				{
					cout<<i<<"号-玩家"<<UNO_get_this_card_color(this_room->player[i].only_color)<<endl;
					color_box[this_room->player[i].only_color-1]++;
				}
			
			cout<<"绿"<<color_box[0]<<"红"<<color_box[1]<<"蓝"<<color_box[2]<<"黄"<<color_box[3]<<endl;
			
			for(int i=0;i<4;i++)
				if(color_box[i]==0)
				{
					cout<<"破釜沉舟!\n";
					p1->i_want_play_this_color=i+1;//选别人没有的颜色，让大家都不得不抽卡
					return;
				}
		}
}
	...
	if(this_room->current_running_effect==UNO_none)//如果当前没有加罚效果
		if(next_player->card_color[3]==UNO_none)//如果下家手牌少于4张
			for(int i=0;i<UNO_deck_card_limit;i++)
			{
				...
				{
					if(UNO_user_apply_play_this_card(this_room,p1,i))//打出同色的功能牌
						return true;
				}
				...
				{
					if(UNO_user_apply_play_this_card(this_room,p1,i))//打出相同效果的功能牌
						return true;
				}
			}
...
bool UNO_AI_careful_follow_play_card(UNO_room* this_room,UNO_player* p1)//谨慎跟牌
{
		for(int i=0;i<UNO_deck_card_limit;i++)
		{
			...
			if((this_room->current_running_effect!=UNO_add_card&&p1->card_color[i]==this_room->current_color)||p1->card_effect[i]==this_room->last_effect)//有匹配的
			{
				...
				if(UNO_user_apply_play_this_card(this_room,p1,i))//把那张牌打出,打出牌函数内自带回合切换
					return true;//作出行动了
			}
...
void UNO_AI_think(UNO_player* p1)
{
	...
	while(true)
	{
		...
		//挂起其他所有人线程
		cout<<"\n";
		cout<<this_room->whose_turn+1<<"号AI开始计算";


		//分析自己手牌，确定自己想出什么颜色牌，如果之后自己遇到黑牌选择情况，选择那个颜色,这个函数在AI每次选择颜色时都会打出，不用在这里使用
		...
		cout<<"2";
		//如果场上有人只剩一张牌且确定only_color和last_color一样，则打出黑牌改变颜色，选择一个自己有的颜色但不是那人的only_color，或者随机改变颜色
